...
def process_task(args):
    ...
    try:
        ...
        if len(parts) >= 3:
            ...
            cursor.execute('''
                INSERT INTO FinalResults (filename, algorithm, p, time, additional_time, max_tries, max_flips, tries, flips, actual_flips)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (cnf_file, 'walksat', p, time_walksat, None, max_tries, max_flips, tries, flips, actual_flips))
            conn.commit()
        else:
    ...
    try:
        ...
        if len(parts) >= 4:
            ...
            cursor.execute('''
                INSERT INTO FinalResults (filename, algorithm, p, time, additional_time, max_tries, max_flips, tries, flips, actual_flips)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (cnf_file, 'wsatA2', p, time_colorwalksat, time_color, max_tries, max_flips, tries, flips, actual_flips))
            conn.commit()
        else:
    ...
    # Run wsatA1.py
    wsatA1_cmd = [
        'python', os.path.join(walksat_path, 'wsatA1.py'),
        '-cnf', cnf_file_path,
        '-p', str(p),
        '--max_tries', str(max_tries),
        '--max_loops', str(max_flips)
    ...
    try:
        wsatA1_output = subprocess.check_output(wsatA1_cmd, universal_newlines=True).strip()
        # Expected output format: time_colorwalksat time_color tries flips
        parts = wsatA1_output.strip().split()
        if len(parts) >= 4:
            time_colorwalksat = float(parts[0])
            time_color = float(parts[1])  # Additional time, e.g., for coloring
            tries = int(parts[2])
            ...
            cursor.execute('''
                INSERT INTO FinalResults (filename, algorithm, p, time, additional_time, max_tries, max_flips, tries, flips, actual_flips)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (cnf_file, 'wsatA1', p, time_colorwalksat, time_color, max_tries, max_flips, tries, flips, actual_flips))
            conn.commit()
        else:
            print(f"Unexpected output from wsatA1.py on {cnf_file} with p={p}: {wsatA1_output}")
    except subprocess.CalledProcessError as e:
        print(f"Error running wsatA1.py on {cnf_file} with p={p}: {e}")


    # Run wsatB.py
    wsatB_cmd = [
        'python', os.path.join(walksat_path, 'wsatB.py'),
        '-cnf', cnf_file_path,
        '-p', str(p),
        '--max_tries', str(max_tries),
        '--max_loops', str(max_flips)
    ...
    try:
        wsatB_output = subprocess.check_output(wsatB_cmd, universal_newlines=True).strip()
        # Expected output format: time_colorwalksat time_color tries flips
        parts = wsatB_output.strip().split()
        if len(parts) >= 4:
            time_colorwalksat = float(parts[0])
            time_color = float(parts[1])  # Additional time, e.g., for coloring
            tries = int(parts[2])
            ...
            cursor.execute('''
                INSERT INTO FinalResults (filename, algorithm, p, time, additional_time, max_tries, max_flips, tries, flips, actual_flips)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (cnf_file, 'wsatB', p, time_colorwalksat, time_color, max_tries, max_flips, tries, flips, actual_flips))
            conn.commit()
        else:
            print(f"Unexpected output from wsatB.py on {cnf_file} with p={p}: {wsatB_output}")
    except subprocess.CalledProcessError as e:
        print(f"Error running wsatB.py on {cnf_file} with p={p}: {e}")
